...
const attachedSlaveClients = new Map<string, PipeClient>()

function addSlaveClient(name: string, client: PipeClient): void {
  attachedSlaveClients.get(name)?.disconnect()
  attachedSlaveClients.set(name, client)
}
